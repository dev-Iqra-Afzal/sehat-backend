...
@router.get("/ngo/{location}")
async def get_ngo_centers(
    request: Request,
    location: str,
):
    try:
        ...
        prompt = (
            f"Give me a list of all ngo centers in {location}. (more than 10) "
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching ngo centers: {str(e)}")
