...
async def get_hospitals(
    ...
    try:
        ...
        prompt = (
            f"Give me a list of all hospitals near {location} or that include the name {location}. (more than 10) "
...
async def get_hospitals(
    ...
    try:
        ...
        print("name: ", name)
        prompt = (
            f"Give me a list of all hospitals that include name {name}. (more than 10) "
            f"Include their name, address, contact information, and working hours. "
            f"Respond in JSON format as a list of dictionaries with keys: name, address, email, contact, hours."
        )
        ...
        # Optional: Validate data format
        for item in data:
            if not all(k in item for k in ("name", "address", "email", "contact", "hours")):
                raise ValueError("Incomplete data format in AI response")

        return {"name": name, "sources": data}

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching hospitals: {str(e)}")


@router.get("/hospitals/data/{name}")
async def get_hospitals(
    request: Request,
    name: str,
):
    try:
        # Construct the AI prompt
        prompt = (
            f"Give me information of the {name} hospital "
            f"Include their name, address, contact information, and working hours. "
            f"Respond in JSON format as a list of dictionaries with keys: name, address, contact, email, hours."
        )
        ...
        # Optional: Validate data format
        for item in data:
            if not all(k in item for k in ("name", "address", "contact", "email", "hours")):
                raise ValueError("Incomplete data format in AI response")

        return {"name": name, "hospital": data}
...
#                     "email": "iqraafzal962@gmail.com",
#                     "hours": "24/7"
#                 }
#             ]
#             return {"name": name, "hospital": dummy_data}
